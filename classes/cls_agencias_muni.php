...
class cls_agencias_muni extends cls_connect {
    static $cursor = NULL;
    static $Error = '0';
    static $message = '';
    static $curr_page = 0;
    ...
    function __construct($id_muni,$dt_compet) {
        parent::__construct();
        ...
        if (self::$conn) {
            ...
            if (self::$id_muni and self::$dt_compet) {
                $sql = "SELECT FC_getTotalAgencias(?, ?) AS ntot FROM dual";
                $result = json_decode($this->dbquery($sql,$id_muni,$dt_compet));
                self::$total = ($result->records[0])->ntot;
                if (self::$total == 0) {
                    self::$Error = '404';
                    self::$message = 'Não existem registros de recolhimento';
                }
            } else {
                self::$Error = '401';
    ...
    public function getCursor() {
        if (self::$connected) {
            return array('Error' => self::$Error, 'Message' => self::$message, 'Data' => self::$total);
        } else {
    ...
    public function getRows($page=1) {
        ...
        if (self::$connected) {
            self::$curr_page = $page;
            $sql = "CALL PR_virecolhimentos(?,?,?,?)";
            $result = json_decode($this->dbquery($sql,self::$id_muni,self::$dt_compet ,$page,self::$limit ));
            if ($result->nrecords > 0) {
                ...
                self::$Error = 0;
                self::$message = 'registros capturados com successo';
            } else {
                self::$Error = 404;
