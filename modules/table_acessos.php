<?php
    ...
    if (is_null($dt_compet)) {
        ...
        $pattern = '/^\d{4}-\d{2}$/';
        if (!preg_match($pattern, $dt_compet)) {
            $dt_compet = NULL;
            $oCompet = new cls_competencias($id_user, $tp_user);
            $dt_compet = $oCompet->getUltimo()['dt_compet'];
            $dt_compet = substr($dt_compet,3) . '-' . substr($dt_compet,0,2);
    ...
    if (! is_null($sort_fields)) {
        foreach ($sort_fields as $field => $value) {
            if (empty($value) or is_null($value)) {
                ...
                if (! in_array($field, array('cd_estado','nm_muni','nm_responsavel', 'cd_status', 'vl_ISS'))) {
                    unset($sort_fields[$field]);
                } else {
    ...
    if (gettype($id_user) != 'integer') {
        if (!is_numeric($id_user)) {
            $id_user = null;
            echo json_encode(['Error' => '505', 'Message' => 'Parametro de pesquisa inválido']);
            $berros = True;
        } else {
            $num_rows = intval($num_rows);
        }
    ...
    if ($berros==False) {
        if (! in_array($tp_user, ['Gestor','Administrador','Sistema', 'Elaborador', 'Validador','Validador+','Financeiro'])) {
            echo json_encode(['Error' => '505', 'Message' => 'Parametro de pesquisa inválido']);
            $berros = True;
        }
    }
    if ($berros==False) {
        ...
        foreach ($vlistMuni as $row) {
            ...
            $qt_agencias = isset($row['tot_agencias']) ? $row['tot_agencias'] : '0';
            ...
            $cline .= "<tr scope='row' class='mw-100 bg-white text-dark'
                            data-idresponsavel='$id_user' data-tpuser='$tp_user' data-idmuni='$id_muni'
                            data-idobrigacao='$id_obrigacaoacessoria' data-idusermunicipio='$id_usermunicipio'
                            data-iddivergencia='$id_divergencia' data-idsistema='$id_idsistema' data-qtagencia='$qt_agencias'>";
            $cline .= "<td scope='col' class='sm-auto'>$cd_estado</td>";
            $cline .= "<td scope='col' onClick='showRecolhimento(event);'><span class='.btRecolhimento' style='display:block; cursor:pointer;'>$nm_muni</span></td>";
            ...
            $cline .= "<td scope='col' style='text-align:center;cursor:hand;'>$dt_obrigacao</td>";
            $cline .= "<td scope='col' style='text-align:center;'>$hr_obrigacao</td>";
            $cline .= "<td scope='col' style='text-align:center;cursor:hand;'>$dt_entregaguia</td>";
            $cline .= "<td scope='col' style='text-align:center;'>$hr_slaentregaguia</td>";
            $cline .= "<td scope='col' style='text-align:center;cursor:hand;'>$dt_vencrecolhimento</td>";
