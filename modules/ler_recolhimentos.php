<?php
    ...
    if ($vresult_esteiras->Error == '0') {
        $vlist_esteiras = $vresult_esteiras->Dados;
    } else {
    ...
    if (!$bsemResultados) {
        foreach ($data as $row) {
            ...
            $vl_desconto = $row['vl_desconto'] ?? 0.0;
            ...
            $sline = "<tr scope='row'  >";
            $sline .= "<td scope='col'><input type='radio' name='sel_agencia' id='sel_agencia-$id_agencia' value='$id_agencia'></td>";
            ...
            foreach ($vlist_esteiras as $row_esteira) {
                $row_esteira = get_object_vars($row_esteira);
            ...
            $sline .= "<td scope='col' class='text-start'><label class='form-control-label text-dark'>$tp_tributo</label></td>";
            ...
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($vl_multa,2,',','.') . "' id = 'vl_multa-$id_agencia'></td>";
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($taxa_esc,2,',','.') . "' id = 'vl_taxa_esc-$id_agencia'></td>";
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($vl_desconto,2,',','.') . "' id= 'vl_desconto-$id_agencia'></td>";
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($vl_arredondamento,2,',','.') . "' id='vl_arredondamento-$id_agencia'></td>";
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($vl_totalarecolher,2,',','.') . "' id='vl_totalarecolher-$id_agencia'></td>";
            $cmd = "<select name='sel_divergencias' id='sel_divergencias-$id_agencia' class='form-select bg-transparent text-dark'>";
            ...
            if ($erro == '0') {
                $cursor = $result['Data'];
                foreach ($cursor as $row_divergencia) {
                    ...
                    if ($cd_tipo == $cd_tipoagencia) {
                        if ($id_currdivergencia == $id_divergencia) {
                            $cmd.= "<option value='$id_currdivergencia' selected>$te_divergencia</option>";
                        } else {
                            $cmd.= "<option value='$id_currdivergencia'>$te_divergencia</option>";
                        }
                    }
                }
            }
            ...
            $sline .= "<td scope='col' class='text-start'>$nm_elaborador</td>";

            $sline .= "<td scope='col' class='text-start'>";
            $sline .= "<input type='text' class='form-control input-sm' id='nu_protocolo-$id_agencia' value='$nu_protocolo' onClick='AnexarProtocolo(event);'>&nbsp;";
            $sline .= "<span><image src='$anexo' width='16' alt='anexo' title='Anexar protocolo'/></span></td>";
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($vl_protocolo,2,',','.') . "' id='vl_protocolo-$id_agencia'></td>";
            $sline .= "<td scope='col' class='text-start'>";
            $sline .= "<input type='text' class='form-control input-sm' id='nu_guia-$id_agencia' value='$nu_guia' onClick='AnexarGuia(event);'>&nbsp;";
            $sline .= "<span><image src='$anexo' width='16' alt='anexo' title='Anexar guia'/></span></td>";
            
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($vl_guia,2,',','.') . "' id='vl_guia-$id_agencia'></td>";
            $sline .= "<td scope='col' class='text-end'><input type='text' class='form-control input-sm' value='" . number_format($vl_correcao_guia ,2,',','.') . "' id='vl_correcao_guia-$id_agencia'></td>";
            $sline .= "<td scope='col' class='text-end'>";
            $cmd = "<select name='sel_divergencias' id='sel_divergencias-$id_agencia' class='form-select bg-transparent text-dark'>";

            $result = $oDivergencia->getCursor($id_recolhimento);
            $erro = $result['Error'];
            ...
            $sline .= "<td scope='col' class='text-start'>$nm_aprovador</td>";

            $sline .= "<td scope='col' class='text-center'>";
            $cmd = "<select name='sel_validacao' id='sel_validacao-$id_agencia' class='form-select bg-transparent text-dark'>";
            ...
            $sline.= $cmd;
            $sline .= "<td scope='col' class='text-center'>";
            $cmd = "<select name='sel_validacaoplus' id='sel_validacaoplus-$id_agencia' class='form-select bg-transparent text-dark'>";
            ...
            $sline.= $cmd;
            $sline .= "<td scope='col' class='text-center'>";
            $cmd = "<select name='sel_justificativa' id='sel_justificativa-$id_agencia' class='form-select bg-transparent text-dark'>";
            $oJustitificativa = new cls_justificativas($cd_justificativa);
            $result = $oJustitificativa->getCursor();
            if ($result['Error'] == '0') {
                $cursor = $result['Data'];
                foreach ($cursor as $row_justificativa) {
                    ...
                    if ($cd_justificativa == $cd_justificativa) {
                        $cmd.= "<option value='$cd_justificativa' selected>$te_justificativa</option>";
                    } else {
                        $cmd.= "<option value='$cd_justificativa'>$te_justificativa</option>";
                    }
                }
            }
            $cmd.= "</select>";
            ...
            $sline .= "</td>";
            $sline .= "<td scope='col' class='text-start'>";
            $sline .= "<textarea class='form-control input-sm' rows='2' cols='25' id='te_justificativa-$id_agencia'>$te_justificativa</textarea></td>";
            $sline .= "<td scope='col' class='text-center'>";
            $sline .= "<image src='$anexo' width='16' alt='anexo' title='Anexar comprovante pgto.' onClick='AnexarComprovante(event);'/></td>";
            $sline .= "<td scope='col' class='text-center'>";
            $sline .= "<div class='form-check'>";
            $lbstatus_pgto = "Não";
            if (empty($nu_protocolo)) {
                $sline .= "<input class='form-check-input' type='checkbox' id='chk_status_pgto-$id_agencia' name='chk_status_pgto' value='$cs_status_pgto' disabled>";
            } else {
                $lbstatus_pgto = "Sim";
                $sline .= "<input class='form-check-input' type='checkbox' id='chk_status_pgto-$id_agencia' name='chk_status_pgto' value='$cs_status_pgto' checked>";
            }
            
            $sline .= "<label class='form-check-label' for='chk_status_pgto-$id_agencia'>$lbstatus_pgto</label>";
            $sline .= "</div></td>";
            $sline .= "<td scope='col' class='text-center'>$cd_contacontabil</td>";
